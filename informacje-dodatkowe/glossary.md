...
**Aplikacja** – Aplikacja Mobilna Partnera, komunikująca się z [SDK Systemu Płatności Online Autopay](../sdk-mobilne/mobile-sdk.md) w celu rejestrowania Transakcji.
...
**IPN** **(Instant Product Notification)** – natychmiastowe powiadomienie wysyłane z Systemu płatności online do Serwisu Partnera przekazujące zmianę statusu produktu. Struktura IPN jest podobna do ITN (rozszerzona jedynie o węzeł _product_).
...
**Link płatności** – żądanie umożliwiające start Transakcji wejściowej (opisanej w części [Rozpoczęcie transakcji](../bramka-platnosci-online/podstawowy-proces-platnosci/start-transaction.md#rozpoczęcie-transakcji)). Można go stosować bezpośrednio na stronach www (metoda POST), natomiast w mailach do Klientów należy posłużyć się _Przedtransakcją_ w celu uzyskania krótkiego linka do płatności (metoda GET).
